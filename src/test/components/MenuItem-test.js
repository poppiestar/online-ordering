...
suite('MenuItem component', () => {
    ...
    test("should fire the action to add the menu item to state", (done) => {
        const addItemToOrderSpy = sinon.spy();
        const wrapper = shallow(<MenuItem addItemToOrder={addItemToOrderSpy} id={props.id} name={props.name} price={props.price} />);

        wrapper.find('input').simulate('change');

        expect(addItemToOrderSpy.callCount).to.equal(1);
        expect(addItemToOrderSpy.calledWith(props.id)).to.equal(true);
        done();
    });
});
